...
export function SocialProofPreview() {
  ...
  const minimumSample = summary?.minimumSample ?? 24;
  const hasEnough = sampleCount >= minimumSample;

  return (
    <section className="mt-8 grid gap-3 rounded-3xl border border-white/10 bg-white/5 p-5 md:grid-cols-2">
      <article className="rounded-2xl border border-white/10 bg-slate-900/60 p-4">
        ...
        <p className="text-xs text-slate-400">{sampleCount} runs in last 7 days</p>
      </article>
      <article className="rounded-2xl border border-white/10 bg-slate-900/60 p-4">
        ...
        <p className="text-xs text-slate-400">Stats unlock after {minimumSample} weekly samples</p>
      </article>
    </section>
  );
}
